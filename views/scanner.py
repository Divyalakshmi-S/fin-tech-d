    ...
    with scan_tab2:
        ...
        if st.button("🔍 Find Cheap Stocks", key="scan_cheap"):
            try:
                ...
                if opps:
                    # Save all suggestions for prediction tracking
                    for o in opps:
                        save_scanner_suggestion(o)

                    # Build portfolio lookup for portfolio-aware suggestions
                    portfolio_by_ticker = {}
                    total_portfolio = 0
                    if holdings:
